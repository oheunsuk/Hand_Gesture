...
DATASET_ROOT = YOLO_DIR / "datasets"
...
def _set_dataset_split(split_name: str) -> None:
    global BASE_PATH, IMG_DIR, LBL_DIR

    if split_name not in VALID_SPLITS:
        raise ValueError(f"지원하지 않는 split입니다: {split_name} (가능값: train, val, test)")

    BASE_PATH = DATASET_ROOT / split_name
...
def migrate_legacy_dataset_names() -> None:
    if LBL_DIR is None or IMG_DIR is None:
        raise RuntimeError("dataset 경로가 초기화되지 않았습니다.")
...
def run_collector(dataset_split: str = "train", class_id: int = 0) -> None:
    _set_dataset_split(dataset_split)
    ...
    print(f"[INFO] 현재 dataset split: {dataset_split}")
    ...
    print(f"=== [{dataset_split.upper()} | {current_class_name.upper()}] 수집 및 테스트 모드 ===")
        ...
        while cap.isOpened():
            ...
            if key == ord("1"):
                ...
                print(
                    ...
                    f"(split:{dataset_split}, 이미지:{image_status}, 클래스:{CLASS_DISPLAY_MAP[current_class_id]})"
                )
            elif key == ord("q"):
                break
    finally:
...
def run_fixed_split_entrypoint(split_name: str) -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("class_id", nargs="?", type=int, choices=[0, 1], default=0)
    args = parser.parse_args()
    run_collector(dataset_split=split_name, class_id=args.class_id)
...
def _main() -> None:
    ...
    parser.add_argument("class_id", nargs="?", type=int, choices=[0, 1], default=0)
    args = parser.parse_args()
    run_collector(dataset_split=args.split, class_id=args.class_id)
